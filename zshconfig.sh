...
else ohmyzsh=$(zenity --list --title="$title" --height=550 --text  "Oh-my-zsh is a configuration for the Z shell, including the option for many themes, plugins, etc.\n\nWould you like to install oh-my-zsh" --radiolist --column "Select" --column "Options" FALSE yes FALSE no)
fi
if [ "$ohmyzsh" = "no" ]
then exit
fi
...
sh -c "$(curl -fsSL https://raw.githubusercontent.com/robbyrussell/oh-my-zsh/master/tools/install.sh)" | zenity --progress --title="$title" --pulsate --no-cancel --auto-close
